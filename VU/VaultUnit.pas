...
{$R *.dfm}
function SendMessage(Text,Caption:string;Flags:Integer):integer;
begin
...
begin
  ...
  begin
    SendMessage(Format(Msg,[item.ItemMdbInfo.nome,inttostr(item.ItemMdbInfo.X)+'x'+inttostr(item.ItemMdbInfo.Y)])+#13+#13+Format(VaultError2,[inttostr(item.ItemMdbInfo.X)+'x'+inttostr(item.ItemMdbInfo.Y)]),VaultErrorCaption,mb_ok+mb_iconwarning);
    result:=false;
    exit;
  end;
  for i:=item.X to (item.X+item.ItemMdbInfo.x-1) do
...
begin
  ...
  if caninsert=false then
  begin
    SendMessage(Format(VaultError1,[item.ItemMdbInfo.nome,inttostr(item.ItemMdbInfo.X)+'x'+inttostr(item.ItemMdbInfo.Y)])+#13+#13+Format(VaultError2,[inttostr(item.ItemMdbInfo.X)+'x'+inttostr(item.ItemMdbInfo.Y)]),VaultErrorCaption,mb_ok+mb_iconwarning);
    result:=false;
    exit;
  end;
end;
...
begin
  ...
  begin
    ...
    if (VaultForm.cbsql.Checked) and (SendMessage(ssql,'Query',mb_yesno+mb_iconinformation)=mrno) then
    begin
      screen.Cursor:=0;
      result:=false;
      exit;
    ...
    begin
      ...
      try
        ...
        on e:exception do
        begin
          SendMessage(e.Message,VaultErrorCaption,mb_ok+mb_iconerror);
          result:=false;
        end;
      end;
    end;
  ...
  begin
    try
      ...
      if VaultVar.ExtraVaults[VaultVar.VaultAtual].Edit=False then
        begin
          ...
          begin
            ...
            inc(j);
            inc(i);
          //  if i=item.datasize-2 then break;
      ...
      for X:=0 to 7 do
       ...
       on E:exception do SendMessage(e.Message,VaultErrorCaption,mb_ok+mb_iconerror);
     end;
  end;
end;
...
begin
  SendMessage(GetItemStringInfo(VaultVar.vaultitems[VaultVar.ItemClick]),ItemInformation,mb_ok);
end;
procedure TVaultForm.N3Click(Sender: TObject);
begin
  timer1.Enabled:=false;
  if SendMessage('VaultQuestion2',programname,mb_yesno+mb_iconquestion)=id_no then
  begin
    timer1.Enabled:=true;
    exit;
  end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if (VaultForm.VaultADOQuery2.Eof) and (VaultForm.VaultADOQuery2.Bof) then
        begin
          SendMessage('VaultError5',VaultErrorCaption,mb_ok+mb_iconerror);
          screen.Cursor:=0;
          VaultForm.VaultADOQuery2.Close;
          exit;
        end;
...
VaultForm.timer1.enabled:=false;
  ...
  if SendMessage('VaultQuestion3',programname,mb_yesno+mb_iconquestion)=id_no then exit;
  VaultForm.timer1.enabled:=true;
  if SalvarWarehouse then
  begin
    SendMessage('VaultConfirm1',programname,mb_ok+mb_iconinformation);
    vaultform.Close;
  end else
  begin
    SendMessage('VaultError8',VaultErrorCaption,mb_ok+mb_iconwarning);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (Sx=cbox.Value-1) and (Sy=cboy.Value-1) then
          itemadd('ws');
        cbox.Value:=sx+1;
        cboy.Value:=sy+1;
      end else
        ...
        begin
          SendMessage(GetItemStringInfo(VaultVar.vaultitems[VerificarVaultItem(VaultImg.UClickEX,VaultImg.UClickEY)]),ItemInformation,mb_ok);
        end;
    end;
    if (VaultVar.VaultOn=false) then
    begin
      if (VaultImg.UClickET='INV') then
        ...
        begin
          ...
          if (Sx=cbox.Value-1) and (Ty=cboy.Value-1) then
            itemadd('inv');
          cbox.Value:=sx+1;
          cboy.Value:=Ty+1;
        end else
          begin
            SendMessage(GetItemStringInfo(VaultVar.inventoryitems[VerificarInvItem(VaultImg.UClickEX,VaultImg.UClickEY)]),ItemInformation,mb_ok);
          end;
      if (VaultImg.UClickET='PS') then
        ...
        begin
          ...
          begin
            SendMessage(GetItemStringInfo(VaultVar.psitems[VerificarpsItem(VaultImg.UClickEX,VaultImg.UClickEY)]),ItemInformation,mb_ok);
          end;
      if (VaultImg.UClickET='EQP') and  ((VaultImg.UClickEY)= VerificarEquipItembypos(pt.x,pt.y)) and (VaultImg.UClickEY<>12) then
        ...
        begin
          ...
          begin
            SendMessage(GetItemStringInfo(VaultVar.equipitems[VerificarEquipItembyPos(pt.x,pt.Y)].ItemInfo),ItemInformation,mb_ok);
...
begin
  ...
  if (VaultImg.Cpt.X<>pt.X) or (VaultImg.Cpt.Y<>pt.Y)  then
  begin
    VaultImg.Cpt:=vaultform.ClientOrigin;
    exit;
  end;
  ...
  Sy:=(Pt.Y div (VaultImg.LagItemY));
  r:=pt.x/VaultImg.LagItemX;
  if r>0 then Sx:=(Pt.X div VaultImg.LagItemX)
    else Sx:=-1;
  r:=Pt.Y/VaultImg.LagItemY;
  if r>0 then Sy:=(Pt.y div VaultImg.LagItemY)
    else Sy:=-1;

  ty:=(Pt.Y-225);
  if ty>0 then
  begin
    r:=ty/VaultImg.LagItemY;
    if r>0 then ty:=ty div (VaultImg.LagItemY)
      else ty:=-1;
  end;
...
begin
  ...
  begin
    ...
    if (VaultForm.VaultADOQuery2.Eof) and (VaultForm.VaultADOQuery2.Bof) then
    begin
      SendMessage('VaultError6',VaultErrorCaption,mb_ok+mb_iconerror);
      VaultForm.VaultADOQuery2.Close;
      exit;
      vaultform.Close;
    end;
